...
# `set -ex` was here. The -x was removed 2026-07-30 because this script takes a
# database password as an argument and passes it to mysql/mysqladmin on the
# command line - so tracing every command printed the password in plaintext to
# stdout, and into any log or terminal capture of a test-setup run.
# -e is kept: a failed step must stop the script.
set -e
